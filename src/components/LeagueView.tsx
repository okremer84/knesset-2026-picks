...
export const LeagueView: React.FC<LeagueViewProps> = ({
  ...
  const benchmarkTurnout = league.benchmarkTurnoutPercentage ?? 70.6;
  ...
  const scoredMembers = league.members.map((member) => {
    ...
    const turnout = member.turnoutPercentage;
    const turnoutDiff =
      typeof turnout === 'number' && !isNaN(turnout)
        ? Math.round(Math.abs(turnout - benchmarkTurnout) * 10) / 10
        : null;
  ...
  // Sort by total score descending, then tiebreaker (exact hits, then closest turnout % to benchmark)
  scoredMembers.sort((a, b) => {
    // 1. Total fantasy score
    if (b.scoreResult.totalScore !== a.scoreResult.totalScore) {
      return b.scoreResult.totalScore - a.scoreResult.totalScore;
    }
    // 2. Exact hits count
    if (b.scoreResult.exactHitsCount !== a.scoreResult.exactHitsCount) {
      return b.scoreResult.exactHitsCount - a.scoreResult.exactHitsCount;
    }
    // 3. Turnout percentage tiebreaker (closer to benchmark turnout wins)
    if (a.turnoutDiff !== null && b.turnoutDiff !== null) {
      if (Math.abs(a.turnoutDiff - b.turnoutDiff) > 0.001) {
        return a.turnoutDiff - b.turnoutDiff;
      }
    } else if (a.turnoutDiff !== null) {
      return -1;
    } else if (b.turnoutDiff !== null) {
      return 1;
    }
    // 4. Overall seat diff (lower is better)
    return a.scoreResult.totalSeatDiff - b.scoreResult.totalSeatDiff;
  });

  // Identify if any member won their spot over another due to the tiebreak
  for (let i = 0; i < scoredMembers.length - 1; i++) {
    const curr = scoredMembers[i];
    const next = scoredMembers[i + 1];
    if (
      curr.scoreResult.totalScore === next.scoreResult.totalScore &&
      curr.scoreResult.exactHitsCount === next.scoreResult.exactHitsCount &&
      curr.turnoutDiff !== null &&
      next.turnoutDiff !== null &&
      curr.turnoutDiff < next.turnoutDiff
    ) {
      curr.tiebreakDecided = true;
    }
  }
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4 px-4">
                          ...
                          {member.turnoutPercentage ? (
                            <div className="text-[11px] text-amber-700 font-bold mt-1 flex items-center gap-1.5">
                              <span>🗳️ שובר שוויון:</span>
                              <span className="bg-amber-100/90 text-amber-900 px-1.5 py-0.5 rounded font-black text-[10px]">
                                {member.turnoutPercentage}% הצבעה
                              </span>
                            </div>
                          ) : null}
                        </td>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            {scoredMembers.length === 0 ? (
              ...
              <div>
                <div className="bg-amber-50/70 border-b border-amber-200/80 px-4 py-2.5 text-xs flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-1.5 font-bold text-amber-900">
                    <span>🗳️ שיעור הצבעה ארצי להכרעת שוויון:</span>
                    <span className="bg-amber-200/90 text-amber-950 px-2 py-0.5 rounded font-black text-xs">
                      {benchmarkTurnout}%
                    </span>
                  </div>
                  <span className="text-[11px] text-amber-800 font-medium">
                    במקרה של שוויון בנקודות הפנטזי, הדירוג מוכרע לטובת מי שהתחזית שלו קרובה יותר לאחוז ההצבעה בפועל.
                  </span>
                </div>
                  ...
                  <table className="w-full text-right text-xs">
                    <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
                      <tr>
                        ...
                        <th className="py-3 px-4 text-center">שובר שוויון (% הצבעה)</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {scoredMembers.map(({ member, scoreResult, turnout, turnoutDiff, tiebreakDecided }, index) => {
                              ...
                              {turnout !== undefined ? (
                                <div>
                                  <div className="flex items-center justify-center gap-1">
                                    ...
                                    {tiebreakDecided && (
                                      <span
                                        className="px-1.5 py-0.5 rounded bg-amber-500 text-white text-[9px] font-black"
                                        title="הכריע שוויון בניקוד בזכות קרבה לשיעור ההצבעה הארצי!"
                                      >
                                        הכריע 🎯
                                      </span>
                                    )}
                                  ...
                                  <div className="text-[10px] text-slate-500 mt-0.5">
                                    {turnoutDiff !== null ? `פער ${turnoutDiff}% מ-${benchmarkTurnout}%` : ''}
                                  </div>
                                </div>
                              ) : (
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white border border-slate-300 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl space-y-5 text-slate-900">
            ...
            {(() => {
              ...
              const mTurnout = inspectingMember.turnoutPercentage;
              const mDiff =
                typeof mTurnout === 'number'
                  ? Math.round(Math.abs(mTurnout - benchmarkTurnout) * 10) / 10
                  : null;

              return (
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-3 text-center sm:text-right">
                  <div>
                  ...
                  <div>
                    <div className="text-xs text-slate-500 font-medium">שובר שוויון (% הצבעה)</div>
                    <div className="text-xl font-black text-amber-700">
                      {mTurnout !== undefined ? `${mTurnout}%` : '—'}
                    </div>
                    {mDiff !== null && (
                      <div className="text-[10px] text-slate-500">
                        פער {mDiff}% מ-{benchmarkTurnout}%
                      </div>
                    )}
                  </div>
                  <div>
